...
public class AddThreeCustomRgbDialog1 extends Dialog implements
OnSeekBarChangeListener {
	...
	public AddThreeCustomRgbDialog1(Context context, Device device,
	...
	private void findID(){
		...
		Tblue.setText(blue + "");
		color = (RelativeLayout) $(R.id.add_color);
		...
		back.setOnClickListener(new View.OnClickListener(){
			public void onClick(View arg0) {
				AddThreeCustomRgbDialog1.this.dismiss();
			}
		});
	...
	private Object $(int id) {
		...
		private void setListener() {
			// TODO Auto-generated method stub
			radius = resBitmap.getWidth() / 2 - 1;
			final int maxwidth = resBitmap.getWidth();
			final int maxheight = resBitmap.getHeight();
			color.setOnTouchListener(new RelativeLayout.OnTouchListener() {
				@Override
				public boolean onTouch(View v, MotionEvent event) {
					...
					if (action == MotionEvent.ACTION_DOWN
						...
						if (((x - radius) * (x - radius) + (y - radius)
								* (y - radius) - radius * radius) <= 0){
							pixel = resBitmap.getPixel(x, y);
							red = Color.red(pixel);
							green = Color.green(pixel);
							blue = Color.blue(pixel);
							tempRed = Color.red(pixel);
							tempGreen = Color.green(pixel);
							tempBlue = Color.blue(pixel);

							if (!popupWindow.isShowing()){
								showCurPointColor.run();
							}
							popupWindow.update(x + p_off_width-radius/2, y + p_off_height,
									-1, -1);
							red = (int) (tempRed * (add_vsb_diaphaneity
									.getProgress() / 100.0));
							green = (int) (tempGreen * (add_vsb_diaphaneity
									.getProgress() / 100.0));
							blue = (int) (tempBlue * (add_vsb_diaphaneity
									.getProgress() / 100.0));
							Tred.setText(red + "");
							Tgreen.setText(green + "");
							Tblue.setText(blue + "");
						...
						if("WF323".equals(device.getLamp().getL_type())){
							...
							if(255 == currentRedColor){currentRedColor = 254;}
							if(255 == currentGreenColor){currentGreenColor = 254;}
							if(255 == currentBlueColor){currentBlueColor = 254;}
							String ido = "03";
